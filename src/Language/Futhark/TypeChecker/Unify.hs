    ...
    subunify ord bound t1 t2 = do
      ...
      let nonrigid v = isNonRigid v constraints
          ...
          unifyTypeArg (TypeArgDim d1 _) (TypeArgDim d2 _) =
            uncurry (onDims bound nonrigid) (swap ord d1 d2)
        ...
        (Array{}, Array{})
          | ShapeDecl (t1_d : _) <- arrayShape t1',
            ...
            Just t2'' <- peelArray 1 t2' -> do
              uncurry (onDims bound nonrigid) (swap ord t1_d t2_d)
...
applySubstInConstraint _ _ (ParamSize loc) = ParamSize loc
...
linkVarToDim usage vn lvl dim = do
  ...
  modifyConstraints $ M.map $ fmap $ applySubstInConstraint vn $ SizeSubst dim
